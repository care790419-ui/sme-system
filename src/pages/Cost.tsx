...
import { thisMonthCST } from '../utils/date'
...
const Cost: React.FC = () => {
  ...
  const [month, setMonth] = useState(() => thisMonthCST())
  ...
  const radarData = catSummary.map(c => ({
    category: c.category,
    執行率: c.budget > 0 ? Math.round((c.actual / c.budget) * 100) : 0,
  }))

  // 動態月份清單（含資料月份 + 當月）
  const availableMonths = useMemo(() => {
    const months = new Set(costItems.map(c => c.month))
    months.add(thisMonthCST())
    return [...months].sort().reverse()
  }, [costItems])

  // 折線/趨勢表：所有有資料的月份
  const allDataMonths = useMemo(() =>
    [...new Set(costItems.map(c => c.month))].sort()
  , [costItems])

  // 跨月類別實際支出
  const catMonthData = useMemo(() => {
    const cats = [...new Set(costItems.map(c => c.category))]
    return cats.map(cat => {
      const monthActuals: Record<string, number> = {}
      let total = 0
      allDataMonths.forEach(m => {
        const v = costItems
          .filter(i => i.category === cat && i.month === m)
          .reduce((s, i) => s + i.actual, 0)
        monthActuals[m] = v
        total += v
      })
      return { cat, monthActuals, total }
    }).filter(r => r.total > 0)
  }, [costItems, allDataMonths])

  // 圓餅圖資料（從 catSummary 計算，非 mockData）
  const pieData = catSummary.filter(c => c.actual > 0).map((c, i) => ({
    name: c.category,
    value: c.actual,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard title="本月預算總額" value={formatNT(totalBudget)} change={`${monthLabel(month)}預算`} changeType="neutral" icon={Layers} iconBg="bg-blue-50" iconColor="text-blue-600" />
        ...
        <div className="p-6">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <div>
                  ...
                  <ResponsiveContainer width="100%" height={280}>
                    <BarChart data={catSummary} layout="vertical" margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                ...
                <div>
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie data={pieData} cx="50%" cy="50%" outerRadius={90} paddingAngle={2} dataKey="value">
                        {pieData.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                      </Pie>
                      <Tooltip formatter={(v: number) => formatNT(v)} />
                      <Legend formatter={value => {
                        const item = pieData.find(e => e.name === value)
                        return item ? `${value} (${totalActual > 0 ? ((item.value / totalActual) * 100).toFixed(1) : 0}%)` : value
                      }} />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tfoot>
                    <tr className="bg-blue-50">
                      <td colSpan={5} className="py-3 px-4 text-sm font-bold text-blue-800 rounded-l-lg">合計</td>
          ...
          {tab === 'analysis' && (
            <div className="space-y-6">
              ...
              <div>
                <h4 className="text-sm font-semibold text-gray-700 mb-4">各月成本趨勢（實際支出）</h4>
                {allDataMonths.length === 0 ? (
                  <p className="text-xs text-gray-400 py-4 text-center">尚無成本資料，請先在明細清單新增項目</p>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50">
                          ...
                          {allDataMonths.map(m => (
                            <th key={m} className="text-right py-3 px-4 text-xs font-semibold text-gray-500">{monthLabel(m)}</th>
                          ))}
                          <th className="text-right py-3 px-4 text-xs font-semibold text-gray-500 rounded-r-lg">合計</th>
                        </tr>
                      </thead>
                      <tbody>
                        {catMonthData.map((row, i) => (
                          <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                            ...
                            {allDataMonths.map(m => (
                              <td key={m} className="py-2.5 px-4 text-right text-gray-600">
                                {row.monthActuals[m] > 0 ? `NT$ ${(row.monthActuals[m] / 1000).toFixed(0)}K` : '—'}
                              </td>
                            ))}
                            <td className="py-2.5 px-4 text-right font-semibold text-gray-700">
                              NT$ {(row.total / 1000).toFixed(0)}K
                            </td>
                          </tr>
                        ))}
                      </tbody>
